...
static void SDL_Overscan(void) {
	...
	if (sdl.overscan_width) {
		...
		// Find four rectangles forming the border
			...
			rect->x = 0; rect->y = 0; rect->w = Uint16(sdl.draw.width+2*sdl.clip.x); rect->h = (Uint16)sdl.clip.y; // top
			if (rect->h > sdl.overscan_width) { rect->y += (rect->h-sdl.overscan_width); rect->h = sdl.overscan_width; }
			if (sdl.clip.x > (Sint16)sdl.overscan_width) { rect->x += (sdl.clip.x-sdl.overscan_width); rect->w -= 2*(sdl.clip.x-sdl.overscan_width); }
			rect = &sdl.updateRects[1];
			rect->x = 0; rect->y = sdl.clip.y; rect->w = (Uint16)sdl.clip.x; rect->h = (Uint16)sdl.draw.height; // left
			if (rect->w > sdl.overscan_width) { rect->x += (rect->w-sdl.overscan_width); rect->w = sdl.overscan_width; }
			rect = &sdl.updateRects[2];
			rect->x = (Sint16)(sdl.clip.x+sdl.draw.width); rect->y = sdl.clip.y; rect->w = (Uint16)sdl.clip.x; rect->h = (Uint16)sdl.draw.height; // right
			if (rect->w > sdl.overscan_width) { rect->w = sdl.overscan_width; }
			rect = &sdl.updateRects[3];
			rect->x = 0; rect->y = (Sint16)(sdl.clip.y+sdl.draw.height); rect->w = (Uint16)(sdl.draw.width+2*sdl.clip.x); rect->h = (Uint16)sdl.clip.y; // bottom
...
Bitu GFX_SetSize(Bitu width,Bitu height,Bitu flags,double scalex,double scaley,GFX_CallBack_t callback) {
	...
	case SCREEN_OPENGLHQ:
		...
		if(!sdl.desktop.fullscreen) {
		    ...
		    if (scalex < scaley)
			height *= (Bitu)(scaley/scalex);
		    else
			width *= (Bitu)(scalex/scaley);
		    sdl.clip.x=(Sint16)((width-sdl.clip.w)/2);
		    sdl.clip.y=(Sint16)((height-sdl.clip.h)/2);
		}
		putenv(scale);
		sdl.surface=SDL_SetVideoMode_Wrap(width,height,bpp,(sdl.desktop.fullscreen?SDL_FULLSCREEN:0)|SDL_HWSURFACE|SDL_ANYFORMAT);
		if (sdl.surface) {
...
dosurface:
		...
		if (sdl.desktop.fullscreen) {
			...
			|| render.xbrz_using
				...
				sdl.clip.x=0;sdl.clip.y=0;
				sdl.surface=SDL_SetVideoMode_Wrap(width, height, bpp, flags);
			}
			if (sdl.surface == NULL) {
			...
			sdl.surface=SDL_SetVideoMode_Wrap(width+2*sdl.overscan_width,height+2*sdl.overscan_width,bpp,(flags & GFX_CAN_RANDOM) ? SDL_SWSURFACE | SDL_RESIZABLE : SDL_HWSURFACE | SDL_RESIZABLE);
#ifdef WIN32
			if (sdl.surface == NULL) {
				SDL_QuitSubSystem(SDL_INIT_VIDEO);
				if (!sdl.using_windib) {
	...
	break;
		...
		if(GCC_UNLIKELY(d3d->bpp16)) {
		    sdl.surface=SDL_SetVideoMode_Wrap(sdl.clip.w,sdl.clip.h,16,sdl.desktop.fullscreen ? SDL_FULLSCREEN|0x40 : SDL_RESIZABLE|0x40);
		    retFlags = GFX_CAN_16 | GFX_SCALING;
		} else {
		    sdl.surface=SDL_SetVideoMode_Wrap(sdl.clip.w,sdl.clip.h,0,sdl.desktop.fullscreen ? SDL_FULLSCREEN|0x40 : SDL_RESIZABLE|0x40);
		    retFlags = GFX_CAN_32 | GFX_SCALING;
		}
...
static void openglhq_init(void) {
	...
	if (control->cmdline->FindExist("-disable_vga_comp")) {
		...
		putenv((char*)("SDL_OPENGLHQ_DOUBLEBUF=1"));
	SDL_QuitSubSystem(SDL_INIT_VIDEO);
...
void change_output(int output) {
	...
	case 1:
#ifdef WIN32
		sdl.surface=SDL_SetVideoMode_Wrap(640,400,0,SDL_HWSURFACE|SDL_HWPALETTE);
...
void GFX_SwitchFullScreen(void) {
	...
	if(menu.startup) {
		Section_prop * sec=static_cast<Section_prop *>(control->GetSection("vsync"));
		if(sec) {
			if(!strcasecmp(sec->Get_string("vsyncmode"),"host")) SetVal("vsync","vsyncmode","host");
		}
	}
#endif //WIN32
}
...
void Drag_Drop( char path_arg[MAX_PATH] ) {
	if(control->SecureMode()) {
		LOG_MSG(MSG_Get("PROGRAM_CONFIG_SECURE_DISALLOW"));
		return;
	}
...
void OpenFileDialog_Img( char drive ) {
	...
	if(DOSBox_Kor())
		OpenFileName.lpstrFilter = "이미지/ZIP 파일(*.ima, *.img, *.iso, *.cue, *.bin, *.mdf, *.zip, *.7z, *.vhd)\0*.ima;*.img;*.iso;*.mdf;*.zip;*.cue;*.bin;*.7z;*.vhd\0모든 파일(*.*)\0*.*\0";
	else
...
void* GetSetSDLValue(bool isget, int target, void* setval) {
	if (target == GETSET_wait_on_error) {
		if (isget) return (void*) sdl.wait_on_error;
		else sdl.wait_on_error = (setval != 0);
	} else if (target == GETSET_desktop_fullscreen) {
		if (isget) return (void*) sdl.desktop.fullscreen;
		else sdl.desktop.fullscreen = (setval != 0);
	} else if (target == GETSET_desktop_doublebuf) {
		if (isget) return (void*) sdl.desktop.doublebuf;
		else sdl.desktop.doublebuf = (setval != 0);
	} else if (target == GETSET_desktop_want_type) {
		if (isget) return (void*) sdl.desktop.want_type;
		else sdl.desktop.want_type = *static_cast<SCREEN_TYPES*>(setval);
#if C_OPENGL
	} else if (target == GETSET_opengl_bilinear) {
		if (isget) return (void*) sdl.opengl.bilinear;
		else sdl.opengl.bilinear = (setval != 0);
#endif
	} else if (target == GETSET_mouse_autoenable) {
		if (isget) return (void*) sdl.mouse.autoenable;
		else sdl.mouse.autoenable = (setval != 0);
#if defined (WIN32)
	} else if (target == GETSET_using_windib) {
		if (isget) return (void*) sdl.using_windib;
		else sdl.using_windib = (setval != 0);
#endif
	} else E_Exit("Unknown GETSET value");
	return 0;
}
#endif
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_SYSWMEVENT : {
			...
			switch( event.syswm.msg->msg ) {
				...
				case WM_DROPFILES: {
					char buff[MAX_PATH];
					DragQueryFile((HDROP)event.syswm.msg->wParam,0,buff,MAX_PATH);
					Drag_Drop(buff);
					break;
				}
				default:
